...
<head>
  <meta charset="UTF-8">
    ...
    <link rel="stylesheet" href="./assets/bookingstyle.css">
...
<script src="./script/bookingjs.js"></script>
<script>
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        $.each(response, function(index, spot) {
          ...
          html += '</div>';
          html += '<div id="p-price">';
          html += '<sup>Rp</sup>' + spot.price + '<br><h6>per hour</h6>';
          html += '</div>';
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if (response.location && response.price) {
        ...
        $("#p-price-content").html('<sup>Rp</sup>' +response.price + '<br><h6>per hour</h6>');
